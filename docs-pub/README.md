...
|---|---|---|
| [The METIS API](api/API.md) | all of them | You want the map. It is an index: pick your surface here, then read its playbook. |
| [METIS API playbook](api/v1-PLAYBOOK.md) | `/api/v1/` | You're integrating with METIS from outside, as an authenticated client. **This is the one you want.** |
| [Public API playbook](api/public-PLAYBOOK.md) | `/public/` | You want the open, unauthenticated, read-only data behind the public pages — for your own frontend, embed or dashboard. |
...
| [Outreach API playbook](api/outreach-PLAYBOOK.md) | `/api/v1/outreach/` | You're calling the Outreach endpoints specifically. |
